    ...
    override suspend fun saveSetting(periodic: Long, delayed: Long) {
        withContext(dispatcher) {
            dataStore.edit { prefs ->
                prefs[REFRESH_PERIOD_KEY] = periodic
                prefs[FIRST_LAUNCH_DELAY_KEY] = delayed
    ...
    override suspend fun readSetting() {
        withContext(dispatcher) {
            dataStore.data.collect { prefs ->
                val periodic = prefs[REFRESH_PERIOD_KEY] ?: SettingContainer.DEFAULT_REFRESH_PERIOD
                val delayed = prefs[FIRST_LAUNCH_DELAY_KEY] ?: SettingContainer.FIST_LAUNCH_DELAY
                _state.update { it.copy(periodic = periodic, delayed = delayed) }
            }
        }
    }
